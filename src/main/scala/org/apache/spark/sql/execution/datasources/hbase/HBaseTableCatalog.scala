...
case class HBaseTableCatalog(
    ...
    val name: String,
    row: RowKey,
    ts: Timestamp, // new  type for time stamp
    sMap: SchemaMap,
    tCoder: String,
    coderSet: Set[String],
  ...
  def getTimestamp: Seq[Field] = ts.fields
  // new type  for time stamp

  def getPrimaryKey= row.keys(0)
  def getColumnFamilies = {
    // commented by Pratap for timestamp
    //sMap.fields.map(_.cf).filter(_ != HBaseTableCatalog.rowKey).toSeq.distinct

    //// Added by Pratap for time stamp
    sMap.fields.map(_.cf).filter(HBaseTableCatalog.isNotReserved).toSeq.distinct
    // new functionality for time stamp
  }
  ...
  //Added Pratap for Timestamp coloumn include
...
//new function added
  ...
  def validateCatalogDef() = {
    ...
    // added Pratap for timestamp column
    if (ts.isPresent) {
...
//Added by Pratap for timestamp column add
...
// new method added for timestamp
...
object HBaseTableCatalog {
  ...
  val rowKey = "rowkey"
  //Added by Pratap time stamp
  // The hbase timestamp field as long
  val timestamp = "timestamp"  // new type added
  ...
  // Addded by Pratap for time stamp
  ...
  def apply(parameters: Map[String, String]): HBaseTableCatalog = {
    ...
    var numReg = parameters.get(newTable).map(x => x.toInt).getOrElse(0)
    //added for retrive table values also
    if(numReg==0){numReg = parameters.get(table).map(x => x.toInt).getOrElse(0)}

    val rKey = RowKey(map.get(rowKey).get.asInstanceOf[String])
    val ts = Timestamp(map.get(timestamp).orNull.asInstanceOf[String])

    // commented by Pratap for as timestamp  added
    //HBaseTableCatalog(nSpace, tName, rKey, SchemaMap(schemaMap), tCoder, coderSet, numReg)

    //Added by Pratap for timestamp column
