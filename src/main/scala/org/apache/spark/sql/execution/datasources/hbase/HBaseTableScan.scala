...
private[hbase] class HBaseTableScanRDD(
  ...
  //Commented by Pratap for timestamp column
...
//Added By pratap fro timestamp column
   ...
   // new method for Timestamp column
    ...
    // commented by Pratap for Time stamp column
    //val unioned = keySeq ++ valuesSeq
    // Addde by Pratap for time stamp column
...
//     Return the row ordered by the requested order
  ...
  def buildRows(fields: Seq[Field], result: Result): Set[Row] = {
   ...
   // val valueSeq: Seq[Map[Long, (Field, Any)]] = fields.filter(!_.isRowKey).map { x =>
    ...
    //Added by Pratap for new Timestamp column
    val tsSeq = {
      if (relation.catalog.ts.isPresent) {
        val f = relation.catalog.getTimestamp.head
        Seq((f, result.rawCells()(0).getTimestamp)).toMap  }
