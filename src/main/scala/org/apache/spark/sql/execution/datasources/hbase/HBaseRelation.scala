...
case class HBaseRelation (
  ...
  /***
    ...
    * @param selColumns
    * @param rddWithKeyCols
    * @param rowKeyDelim
    * @return
    */
  def getRowsOfKeys(keyCols:Array[String],
                    schema:StructType,
                    nameSpace:String,
                    hbaseTableName:String,
                    selColumns: (String, Array[String]),
                    rddWithKeyCols:RDD[Row],
                    rowKeyDelim:String = ""):RDD[Row] = {
    rddWithKeyCols.mapPartitions(rowIterator => {
      val hbaseConn = HBaseConnectionCache.getConnection(hbaseConf)
      val hbaseTable: Table = hbaseConn.getTable(TableName.valueOf(nameSpace,
        hbaseTableName))
      val rowList = new mutable.MutableList[Row]
      try {
        val colFamily = selColumns._1
        val colArray = selColumns._2
        rowIterator foreach { row => {
          val rowKey = keyCols.map(col => row.getAs[String](col)).mkString(rowKeyDelim)
          val getWithKey: Get = new Get(Bytes.toBytes(rowKey))
          val res: Result = hbaseTable.get(getWithKey)
          if (res == null || res.isEmpty) {
            val errorMsg = "getRowsOfKeys(): No row found for rowkey " + rowKey + " in [" +
            ":" + hbaseTableName + "]"
            logError(errorMsg)
          }
          else {
            val colValueList = new mutable.MutableList[Any]
            try {
              colArray foreach { col =>
                val colField = schema(col.toLowerCase)
                var cellValue:Any = None
                if (res.containsColumn(Bytes.toBytes(colFamily), Bytes.toBytes(col))) {
                  val cell = res.getColumnLatestCell(Bytes.toBytes(colFamily), Bytes.toBytes(col))
                  cellValue = colField.dataType match {
                    case StringType => Bytes.toString(cell.getValue)
                    case x: DecimalType => Bytes.toBigDecimal(cell.getValue)
                    case IntegerType => Bytes.toInt(cell.getValue)
                    case FloatType => Bytes.toFloat(cell.getValue)
                    case BooleanType => Bytes.toBoolean(cell.getValue)
                    case LongType => Bytes.toLong(cell.getValue)
                    case ShortType => Bytes.toShort(cell.getValue)
                    case DoubleType => Bytes.toDouble(cell.getValue)
                ...
                colValueList += cellValue
              } //colArray
              val rowSeq = colValueList.toSeq
              rowList += Row(rowSeq: _*)
            }
            catch {
              case e:Exception => {
                val errorMsg = "Error in converting column value for rowkey " + rowKey + " in [" +
                  ":" + hbaseTableName + "]"
                logError(errorMsg)
              }
            }
  ...
  // commented by Pratap for time stamp
  /*
  ...
  //Added Bt Pratap for Timestamo column to be include
...
// Added new method for time stamp
